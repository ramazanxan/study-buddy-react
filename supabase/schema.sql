-- StudyBuddy — Supabase schema v2
-- ИНСТРУКЦИЯ: Скопируй ВСЁ содержимое этого файла,
-- вставь в SQL Editor на supabase.com и нажми RUN (зелёная кнопка).
...
-- Profiles
create policy "profiles readable by auth"     on public.profiles for select using (auth.role() = 'authenticated');
create policy "profiles public login lookup"  on public.profiles for select using (true);  -- нужно для входа по логину
...
-- ── RPC: найти email по логину (для входа по username) ──────────────
